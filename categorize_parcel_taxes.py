...
df = read_func('consolidated_tax_data_2023_20240329105204')

# zip code & city validation
zips = read_func('uszips.csv').select(col('zip'), col('city').alias('zip_city'), 'state_id')
...
valid_df = (
    ...
    .join(
        ...
        how='left',
        on=(
            (trim(col('a.situs_zip'))==trim(col('b.zip'))) 
            & (lower(col('a.City'))==lower(col('b.zip_city')))
            )
    )
    .withColumn
    (
    ...
    .drop('zip', 'zip_city', 'state_id')
)

# add city col
# raw_100 = read_func('raw_100')
# df = df.join(raw_100,on='parcel_no', how='left').select(df['*'], raw_100['SITUS_CITY'].alias('City'))
...
write_func(valid_df, f'consolidated_tax_data_2023_{current_time}')