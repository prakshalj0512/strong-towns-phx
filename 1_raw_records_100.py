...
df = ans.filter(col('record_type')==100)

df = df.withColumn('acres_clean', col('acres')/43560)
df = df.withColumn('situs_zip', substring('situs_zip', 0, 5))
df = df.withColumn('property_use_code', lpad(df['property_use_code'], 4, '0'))
df = df.withColumn('property_use_code_major', substring('property_use_code', 0, 2))
...
df = df.filter(~col('property_use_code').isNull())

write_func(df, 'bronze/raw_100')
